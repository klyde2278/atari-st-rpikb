...
/*
 * USB HID -> Atari ST IKBD scancode lookup (128 entries).
 * Maps HID usage codes (physical key positions) to ST scancodes.
...
extern "C" {
const uint8_t st_key_lookup_hid[ST_KEY_LOOKUP_SIZE] = {
  /* 0x00..0x03 */ 0,0,0,0,
  ...
  /* 0x44..0x45 F11, F12: unmapped (ST has no F11/F12; these are reserved
     system shortcuts — Ctrl+F11 IKBD reset, Ctrl+F12 mouse/joy toggle —
     and must not alias to keypad '(' / ')' (that's PageUp/PageDown below) */
  0,0,
