...
ps4_controller_t* ps4_get_controller_by_index(uint8_t index) {
    // controllers[] is kept in connection order: allocate_controller()
    // appends and free_controller() compacts the array.
    if (index >= controller_count) {
        return NULL;
    }
    return controllers[index].connected ? &controllers[index] : NULL;
}

void ps4_to_atari(const ps4_controller_t* ps4,
...
void ps4_mount_cb(uint8_t dev_addr) {
    // Runs inside tuh_task() on core 0 — must not block: USB polling and
    // UART RX from the ST are stalled while in here (the previous OLED
    // splash + 1.5 s sleep was long enough to lose serial bytes).
    // The standard UI already reflects the new joystick count via
    // usb_connect_state().
